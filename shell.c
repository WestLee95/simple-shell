...
/* Global Variables */
static char **cmd_history;          /* Command history array */
static int cmd_history_len = 0;     /* Number of commands in history */
static char *user_input = NULL;     /* Input from the user */

static struct command *current_cmd; /* Parent command for cleanup */
static struct commands *cmd_pipeline;
static char *temp_input_line;       /* Temporary input holder */

/* Check if input command is related to history */
int is_history_command(char *input) {
    const char *history_cmd = "history";
    
    if (strlen(input) < strlen(history_cmd))
        return 0;

    for (int i = 0; i < (int)strlen(history_cmd); i++) {
        if (input[i] != history_cmd[i])
            return 0;
    }
    return 1;
}
...
/* Placeholder for further functions like command execution, history handling, etc. */
